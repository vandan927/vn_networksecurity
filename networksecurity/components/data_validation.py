...
class DataValidation:
    ...
    def validate_numerical_columns(self,dataframe: pd.DataFrame) -> bool:
        try:
            numerical_col_df = dataframe.select_dtypes(include='number').columns.tolist()
            if numerical_col_df == self._schema_config['numerical_columns']:
                return True
            else:
                return False
        except Exception as e:
    ...
    def initiate_data_validation(self)-> DataIngestionArtifact:
        try:
            ...
            if not status:
                error_message = f"Train Dataframe numerical columns mismatch.\n"
            
            status=self.validate_numerical_columns(dataframe=test_dataframe)
            if not status:
                error_message = f"Test Dataframe numerical columns mismatch.\n"              
...
#     numerical_col_df = dataframe.select_dtypes(include='numeric').columns.tolist()